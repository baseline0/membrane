...
**TL;DR:** Automated fetcher gets ~10-20% of papers (those with public direct links). The rest need manual collection. System handles both seamlessly: everything goes to `docs/papers/` and gets tracked in manifest.
...
- **Liu et al. (2021)** — IEEE Access
  - Fetched: 37KB
  - Cached: `docs/papers/Liu_NovelHybridQuantum_2021.pdf`
...
3. Download PDF
4. Save to: `docs/papers/AuthorLastName_ShortTitle_Year.pdf`

**Example:**
```bash
# Download Zhang 2023, save as:
docs/papers/Zhang_QuantumInspiredSurvey_2023.pdf
```
...
```json
{
  "Zhang_QuantumInspiredSurvey_2023": {
    ...
    "success": true,
    "source": "manual",
    "filepath": "docs/papers/Zhang_QuantumInspiredSurvey_2023.pdf",
...
```bash
# Just copy/download PDF to docs/papers/
cp ~/Downloads/paper.pdf docs/papers/Author_Title_Year.pdf
...
**A:** No. `paper_status.py` will show any PDFs in `docs/papers/` regardless. Manifest is optional — it just adds provenance (which source, when, hash). For manual papers, you can skip it or add an entry for bookkeeping.
...
```bash
# Check manifest
cat docs/papers/.manifest.json | grep -i "author name"
...
| Want to force-refetch a paper | Delete from `docs/papers/` and from `.manifest.json`, re-run |
